...
def inject_globals():
    ctx = {"current_user": session.get("username", ""), "shops_list": []}
    try:
        ctx["shops_list"] = get_all(COL_SHOPS)
        if "user_id" in session:
            user = get_doc(COL_USERS, session["user_id"])
            if user:
                ctx["current_user"] = user.get("full_name", user.get("username", ""))
    except:
        pass
    return ctx
...
def dashboard():
    ...
    return render_template("dashboard.html",
        total_products=total_products, total_qty=total_qty,
        low_stock_count=low_stock_count, recent_in=si[:5], recent_out=so[:5],
    )
...
def add_stock_in():
    ...
    add_doc(COL_STOCK_IN, generate_id("SI", COL_STOCK_IN), {
        ...
        "qty": qty, "supplier": sanitize(request.form.get("supplier", "")),
        "purchase_date": request.form.get("purchase_date", today_str()),
        "added_by": session.get("username", ""), "timestamp": now_str(),
    })
...
def add_stock_out():
    ...
    add_doc(COL_STOCK_OUT, generate_id("SO", COL_STOCK_OUT), {
        "product_id": product_id, "product_name": product["name"],
        "qty": qty, "customer": sanitize(request.form.get("customer", "")),
        "reason": request.form.get("reason", "sale"),
        "removed_by": session.get("username", ""), "timestamp": now_str(),
    })
...
def pos():
    ...
    if selected_shop:
        ...
        for sk in get_all(COL_SHOP_STOCK):
            ...
            if prod:
                products.append({
                    "id": sk.get("product_id"), "name": prod.get("name", ""),
                    "price": sk.get("price", prod.get("selling_price", 0)),
                    "qty": sk.get("qty", 0), "unit": prod.get("unit", "pcs"),
                })
    ...
    return render_template("pos.html",
        shops=shops, selected_shop=selected_shop,
        selected_shop_name=selected_shop_name, products=products, sales=sales,
        is_admin=is_admin, is_auto_selected=is_auto_selected,
    )
...
def pos_sell():
    ...
    add_doc(COL_SALES, receipt_id, {
        "receipt_id": receipt_id, "shop_id": shop_id,
        "shop_name": get_shop_name(shop_id), "items": items,
        "grand_total": grand_total, "customer": customer,
        "sold_by": session.get("username", ""), "timestamp": now_str(),
    })
...
def add_shop():
    ...
    add_doc(COL_SHOPS, generate_id("S", COL_SHOPS), {
        "name": name, "location": sanitize(request.form.get("location", "")),
        "salesman": sanitize(request.form.get("salesman", "")), "date_created": today_str(),
    })
...
def shop_products(sid):
    ...
    for sk in get_all(COL_SHOP_STOCK):
        ...
        if prod:
            ...
            qty = sk.get("qty", 0)
            products.append({"id": sk.get("product_id"), "name": prod.get("name", ""), "shop_qty": qty, "unit_price": unit_price})
    ...
    return render_template("shop_products.html",
        shop=shop["name"], shop_id=sid,
        warehouse_products=warehouse_products, products=products,
        grand_total=grand_total, records=trans,
    )
...
def _transfer_to_shop(shop_id, product_id, qty, transferred_by):
    ...
    for sk in get_all(COL_SHOP_STOCK):
        if sk.get("shop_id") == shop_id and sk.get("product_id") == product_id:
            ...
            break
    if found:
        update_doc(COL_SHOP_STOCK, found["id"], {"qty": found.get("qty", 0) + qty, "price": product.get("selling_price", 0)})
    else:
        add_doc(COL_SHOP_STOCK, generate_id("SS", COL_SHOP_STOCK), {
            "shop_id": shop_id, "product_id": product_id,
            "qty": qty, "price": product.get("selling_price", 0),
        })
    add_doc(COL_TRANSFERS, generate_id("TF", COL_TRANSFERS), {
        "shop_id": shop_id, "shop_name": get_shop_name(shop_id),
        "product_id": product_id, "product_name": product["name"],
        "qty": qty, "transferred_by": transferred_by, "timestamp": now_str(),
    })
...
def distribute():
    ...
    return render_template("distribute.html",
        shops=get_all(COL_SHOPS), products=get_all(COL_PRODUCTS), records=records,
    )
...
def add_user():
    ...
    add_doc(COL_USERS, generate_id("U", COL_USERS), {
        ...
        "password": hash_password(password), "role": role, "date_created": today_str(),
    })
...
def _report_data(title, headers, data, date_from="", date_to="", with_shops=False):
    return render_template("reports_table.html",
        title=title, headers=headers, data=data,
        date_from=date_from, date_to=date_to,
        shops=get_all(COL_SHOPS) if with_shops else [],
    )
...
def report_stock():
    ...
    data = [{"ID": p.get("id", ""), "Name": p.get("name", ""), "Category": p.get("category", ""), "Unit": p.get("unit", ""), "Buying Price": p.get("buying_price", 0), "Selling Price": p.get("selling_price", 0), "Quantity": p.get("quantity", 0), "Reorder Level": p.get("reorder_level", 0)} for p in get_all(COL_PRODUCTS)]
...
def report_low_stock():
    ...
    data = [{"ID": p.get("id", ""), "Name": p.get("name", ""), "Category": p.get("category", ""), "Quantity": p.get("quantity", 0), "Reorder Level": p.get("reorder_level", 0)} for p in items]
...
def report_stock_in():
    ...
    filtered = [r for r in records if (not date_from or r.get("timestamp", "")[:10] >= date_from) and (not date_to or r.get("timestamp", "")[:10] <= date_to)]
    headers = ["Transaction ID", "Product", "Quantity", "Supplier", "Purchase Date", "Added By", "Timestamp"]
    data = [{"Transaction ID": r.get("id", ""), "Product": r.get("product_name", ""), "Quantity": r.get("qty", 0), "Supplier": r.get("supplier", ""), "Purchase Date": r.get("purchase_date", ""), "Added By": r.get("added_by", ""), "Timestamp": r.get("timestamp", "")} for r in filtered]
    return _report_data("Stock In Report", headers, data, date_from, date_to, with_shops=True)
...
def report_stock_out():
    ...
    data = [{"Transaction ID": r.get("id", ""), "Product": r.get("product_name", ""), "Quantity": r.get("qty", 0), "Customer": r.get("customer", ""), "Reason": r.get("reason", ""), "Removed By": r.get("removed_by", ""), "Timestamp": r.get("timestamp", "")} for r in filtered]
    return _report_data("Stock Out Report", headers, data, date_from, date_to, with_shops=True)
...
@login_required
def seed_sample():
    for pid, name, cat, unit, bp, sp, qty, rl in [
        ...
        if not get_doc(COL_PRODUCTS, pid):
            add_doc(COL_PRODUCTS, pid, {
                ...
                "buying_price": bp, "selling_price": sp, "quantity": qty, "reorder_level": rl,
            })
    for sid, name, loc, salesman in [
        ...
        if not get_doc(COL_SHOPS, sid):
            add_doc(COL_SHOPS, sid, {"name": name, "location": loc, "salesman": salesman, "date_created": today_str()})
    if not any(u.get("username") == "admin" for u in get_all(COL_USERS)):
        add_doc(COL_USERS, "U001", {
            "full_name": "System Admin", "username": "admin",
            "password": hash_password("admin123"), "role": "admin", "date_created": today_str(),
        })
